...
public class DetectAndDrawPupilFilter extends Filter{
	...
	protected Bundle execute(Bundle arg0) {
		...
		detectPupilAndDraw(rgba.getNativeObjAddr(), gray.getNativeObjAddr(), pupilRoiRect, detectedCircles);
		
		Log.i(RGB2GRAYFilter.TAG, this.getFilterName() + "start");
		Bitmap bitmap = Bitmap.createBitmap(rgba.cols(), rgba.rows(), Config.ARGB_8888);
